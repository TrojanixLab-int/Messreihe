...
class MessreiheApp:
    ...
    def setup_ui(self):
        ...
        for p in ["P1", "P2", "P3"]:
            ...
            self.add_tooltip(btn, "Lange drücken zum Speichern")
        ...
        tk.Label(tot_f, text="Gesamt (α+β+γ)", fg=FG_W, bg=BG).pack()
        ...
        self.action_btns = {}
        confs = [
            ("odl", "ODL - Hintergrundstrahlung", "Σ", ["Zeit (s)", "Zerfälle", "CPM", "% (Σαβγ)", "Sv/h (ODL)"]),
            ("alpha", "Alpha-Nuklide", "α", ["Zeit (s)", "Zerfälle", "CPM", "% (αβγ)", "Gy/h (α+β+γ)", "Gy/h (α)"]),
            ("beta", "Beta-Teilchen", "β", ["Zeit (s)", "Zerfälle", "CPM", "% (αβγ)", "Gy/h (β+γ)", "Gy/h (β)"]),
            ("gamma", "Gamma-Strahlung", "γ", ["Zeit (s)", "Zerfälle", "CPM", "% (αβγ)", "Gy/h ((β)+γ)", "Gy/h (γ)"])
        ]
...
Sind Preset-Buttons, mit denen man alle eingegebenen Werte auf der rechten Seite des Programms als Voreinstelungen speichern (> 3 Sekunden drücken) oder laden (einfaches klicken) kann.
...
Typische Standardwerte von gängigen Zählrohren werden häufig folgende Orientierungswerte für 1 µSv/h herangezogen, bei denen man aufgrund der Alpha-Detektions-Effizienz einen weiteren Korrekturfaktor bei der Alpha-Messung berücksichtigen muss:
    ...
    Si8B: 474 CPM (25% Alpha-Effizienz)
    LND-712 (Gammascout): 167 CPM (10% Alpha-Effizienz)
...
Folgende Zählrohre sind weit verbreitet, sind jedoch für Alpha-Nuklide undurchlässig und detektieren diese nicht:
    ...
    ZP1320 (Bundeswehr-Standard): 27 CPM
    ...
    def p_release(self, k):
        if hasattr(self, "p_start"):
            if (time.time() - self.p_start) >= 3.0:
                self.save_config(k)
                messagebox.showinfo("OK", f"Parameter in Preset {k} gespeichert.")
            else:
