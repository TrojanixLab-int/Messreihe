...
class MessreiheApp:
    ...
    def setup_ui(self):
        ...
        for p in ["P1", "P2", "P3"]:
            ...
            self.add_tooltip(btn, "Mantener para guardar")
        ...
        self.action_btns = {}
        confs = [
            ("odl", "TDAA - Tasa de Dosis Ambiental Equivalente", "Σ", ["tiempo (s)", "pulsos", "CPM", "% (Σαβγ)", "Sv/h (TDAA)"]),
            ("alpha", "Núclidos alfa", "α", ["tiempo (s)", "pulsos", "CPM", "% (αβγ)", "Gy/h (α+β+γ)", "Gy/h (α)"]),
            ("beta", "Partículas beta", "β", ["tiempo (s)", "pulsos", "CPM", "% (αβγ)", "Gy/h (β+γ)", "Gy/h (β)"]),
            ("gamma", "Radiación gamma", "γ", ["tiempo (s)", "pulsos", "CPM", "% (αβγ)", "Gy/h ((β)+γ)", "Gy/h (γ)"])
        ]
...
Son botones de preajuste con los que se pueden guardar todos los valores introducidos en el lado derecho del programa como preajustes (presionar > 3 segundos) o cargarlos (clic simple).
...
Si8B: 474 CPM (25% eficiencia alfa)
LND-712 (Gammascout): 167 CPM (10% eficiencia alfa)
...
ZP1320 (Estándar de las Fuerzas Armadas Alemanas): 27 CPM
    ...
    def p_release(self, k):
        if hasattr(self, "p_start"):
            if (time.time() - self.p_start) >= 3.0:
                self.save_config(k)
                messagebox.showinfo("OK", f"Parámetros guardados en el preset {k}.")
            else:
