...
class MessreiheApp:
    ...
    def setup_ui(self):
        ...
        for p in ["P1", "P2", "P3"]:
            btn = tk.Button(self.footer, text=f"П.{p[1]}", bg="#ddd", fg=FG_B, font=("Arial", 9, "bold"), padx=10)
            btn.pack(side="left", padx=2, pady=5)
            btn.bind("<ButtonPress-1>", lambda e, x=p: self.p_press(x))
            btn.bind("<ButtonRelease-1>", lambda e, x=p: self.p_release(x))
            self.add_tooltip(btn, "Удерживайте для сохранения")
        ...
        tk.Label(tot_f, text="Итого (α+β+γ)", fg=FG_W, bg=BG).pack()
        ...
        self.action_btns = {}
        confs = [
            ("odl", "МАЭД - Мощность амбиентного эквивалента дозы", "Σ", ["Время (с)", "импульсы", "отсч/мин", "% (Σαβγ)", "Зв/ч (МАЭД)"]),
            ("alpha", "Альфа-нуклиды", "α", ["Время (с)", "импульсы", "отсч/мин", "% (αβγ)", "Гр/ч (α+β+γ)", "Гр/ч (α)"]),
            ("beta", "Бета-частицы", "β", ["Время (с)", "импульсы", "отсч/мин", "% (αβγ)", "Гр/ч (β+γ)", "Гр/ч (β)"]),
            ("gamma", "Гамма-излучение", "γ", ["Время (с)", "импульсы", "отсч/мин", "% (αβγ)", "Гр/ч ((β)+γ)", "Гр/ч (γ)"])
        ]
...
Это кнопки предустановок, с помощью которых можно сохранить все значения, введенные в правой части программы, в качестве предустановок (удерживать > 3 секунд) или загрузить их (одинарное нажатие).
...
СИ8Б: 474 отсч/мин (25% α-эффективность)
LND-712 (Gammascout): 167 отсч/мин (10% α-эффективность)
...
ZP1320 (стандарт Бундесвера): 27 отсч/мин
...
Функция «Данные нуклидов» открывает внешнее приложение Nukliddaten.exe. Это программное обеспечение стороннего разработчика, которое не является частью данного программного пакета и разрабатывалось независимо. Разработчик СерияМес не несет ответственности за содержание, функции или отсутствие ошибок в этом внешнем ПО. Ссылка служит исключительно для удобства пользователя как физический справочник."""
    ...
    def generate_txt(self, ents, win):
        ...
        with open(path, "w", encoding="utf-8") as f:
            ...
            f.write(f"Доля измерения: {self.v_lbls['alpha'][3].cget('text')} % (без МАЭД)\n")
            ...
            for k, n in [("beta", "Бета-частицы (распады электронов или позитронов)"), ("gamma", "Гамма-излучение (распады гамма-фотонов)")]:
                ...
                f.write(f"Доля измерения: {self.v_lbls[k][3].cget('text')} % (без МАЭД)\n")
    ...
    def p_release(self, k):
        if hasattr(self, "p_start"):
            if (time.time() - self.p_start) >= 3.0:
                self.save_config(k)
                messagebox.showinfo("OK", f"Параметры сохранены в профиле {k}.")
            else:
